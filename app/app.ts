...
import { parseMinute, parseHour, parseDayOfMonth, parseMonth, parseDayOfWeek, parseYear } from './parser';

const getCronParts = (string: string) => {
  const [minutes, hours, dayOfMonth, monthOfYear, dayOfWeek, year] = string.split(' ');
...
const isAnyValueEmpty = (collection: Array<Array<number>>) => !!collection.find((values) => values.length === 0);
...
export const isActive = (cronString: string, matchTime = new Date()) => {
  ...
  const monthOfYear = parseMonth(cronParts.monthOfYear, matchTime).map((m) => m - 1);
  const dayOfWeek = parseDayOfWeek(cronParts.dayOfWeek, matchTime);
  const year = parseYear(cronParts.year, matchTime);

  const activeValues = getActiveBasedOnDate([minutes, hours, daysInMonth, monthOfYear, dayOfWeek, year], matchTime);
